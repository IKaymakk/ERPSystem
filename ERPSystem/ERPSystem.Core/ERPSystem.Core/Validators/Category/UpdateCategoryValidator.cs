...
public class UpdateCategoryValidator : AbstractValidator<UpdateCategoryDto>
{
    ...
    public int CategoryId { get; set; } // Controller'dan set edilecek
    ...
    public UpdateCategoryValidator(ICategoryRepository categoryRepository)
    {
        ...
        RuleFor(x => x.ParentCategoryId)
            ...
            .MustAsync(NotCreateCircularReference).WithMessage("Döngüsel referans yaratılamaz")
            .When(x => x.ParentCategoryId.HasValue);
    }

    private async Task<bool> BeUniqueCode(string code, CancellationToken cancellationToken)
    {
        return !await _categoryRepository.ExistsByCodeAsync(code, CategoryId);
    }

    private async Task<bool> BeUniqueName(string name, CancellationToken cancellationToken)
    {
        return !await _categoryRepository.ExistsByNameAsync(name, CategoryId);
    }
    ...
    private async Task<bool> NotCreateCircularReference(int? parentId, CancellationToken cancellationToken)
    {
        if (!parentId.HasValue) return true;

        // Kendisini parent yapamaz
        if (parentId.Value == CategoryId) return false;

        // Alt kategorilerinden birini parent yapamaz
        return !await _categoryRepository.IsDescendantOfAsync(parentId.Value, CategoryId);
    }
}